...
root_path = os.path.join(os.path.dirname(__file__))#,'OpenVoice_with_voice_clone')
...
def clone_voice_to_sample(sample,voice,device):
    global tone_color_converter
    # this function is not user
    return None
...
def voice_clone_samples_old(device_id,samples,voices_list, override=False, skip_list=None):
    ...
    for voice in voices_list:
        ...
        for sample in samples:
            ...
            cloned_path = os.path.join(ROOT_DIR,'audio','clone',f'{sample_name}_openvoice_{voice_name}{ext}')
            if f'{sample_name}_openvoice_{voice_name}{ext}' in skip_list:
            ...
            duration = end - start
            logging.info(f"Ended voice cloning with OpenVoice for sample {filename} with voice {voice_name}, time: {duration}, device: {device}")
...
def voice_clone_samples(device_id,sample_voice_tuple_list, override=False, skip_list=None):
    ...
    for sample, voice in sample_voice_tuple_list:
        ...
        with torch.no_grad():
            ...
            tone_color_converter.convert(audio_src_path = sample,
                                         src_se = source_se,
                                         tgt_se = target_se,
